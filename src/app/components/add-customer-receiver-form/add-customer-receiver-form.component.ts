...
export class AddCustomerReceiverFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.subscription.add(this.appService.sharedData.customerReceiverForm.eventEmitter.subscribe((v) => {
      switch (v) {
        case 'canCloseDialog?':
          ...
          if (closeDialog) {
            this.appService.sharedData.customerReceiverForm.eventEmitter.emit('closeDialog');
          }
          break;
        default:
          break;
    ...
    debugger;
    if(this.appService.sharedData.customerReceiverForm.isEditMode){
  ...
  private initRoleBasedUI() {
    ...
    } else if (this.appService.sharedData.customerReceiverForm.isViewMode) {
      // view mode
  ...
  private init() {
    ...
    if (this.appService.sharedData.customerReceiverForm.isViewMode || this.appService.sharedData.customerReceiverForm.isEditMode) {
      const dataItem = this.appService.sharedData.customerReceiverForm.dataItem;
    ...
    if (this.appService.sharedData.customerReceiverForm.isViewMode) {
  ...
  private fetchReceiptEmployees() {
    const dateItem = this.appService.sharedData.customerReceiverForm.dataItem;
  ...
  onChangeBehalfOfCusotmer() {
    if (this.appService.sharedData.customerReceiverForm.dataItem) {
      this.appService.sharedData.customerReceiverForm.dataItem.behalfID = this.behalfOfCusotmerSelected?.CustomerID;
      this.fetchDevicesByCustomer();
    }
  }
  ...
  getInvUserByRole(){
    ...
    this.apiService.getInvUserByRole(filterKey, isActive, condition).subscribe({
      next: (v:any) => {
        debugger;
        this.PMReceivers = v;
        if(this.appService.sharedData.customerReceiverForm.isEditMode || this.appService.sharedData.customerReceiverForm.isViewMode)
