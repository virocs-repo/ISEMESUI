...
import { Receipt, ICON, MESSAGES, Customer, KeyValueData, ReceiptLocation, DeliveryMode, ReceiptStatus, ReceivingTypes } from 'src/app/services/app.interface';
...
export class SearchReceivingComponent implements OnDestroy {
  ...
  selectedReceivingTypes: ReceivingTypes[] = [];
  ...
  async ngOnInit(): Promise<void> {
    setTimeout(() => {
      ...
      this.receivingTypes = this.appService.masterData.receivingTypes;
  ...
  tagDisplayLimit(tags: ReceivingTypes[]): ReceivingTypes[] {
      const maxVisibleTags = 1;
      return tags.length > maxVisibleTags
        ? [...tags.slice(0, maxVisibleTags), { serviceCategoryId: 0, serviceCategoryName: `+${tags.length - maxVisibleTags}` }]
        : tags;
    }
    tagDisplayLimit1(tags: ReceiptStatus[]): ReceiptStatus[] {
      const maxVisibleTags = 1;
      return tags.length > maxVisibleTags
  ...
  private fetchdata() {
    ...
    const receivingTypeString = this.selectedReceivingTypes?.length? this.selectedReceivingTypes.map((x: any) => x.serviceCategoryId).join(','): null;    
    this.apiService.getReceivingSearchData(receivingTypeString,this.fromDate,this.toDate,statusString).subscribe({
      next: (v: any) => {
        this.originalData = v;
        this.pageData();
      },
